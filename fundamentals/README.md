...
- Note the `:nth-child(X)` pseudo-class. This selector matches with an element based on its position in its parent. This means if there are 6 `div` children and we only want to write style rules for the 3rd, we would write `div:nth-child(3)`
...
After this change, your browser will look like this:
...
- Because the `div` elements’ widths have collapsed, they no longer take up the full width of the `section`, and we can see the black background of the element. Also, note how the height of the `section` element has collapsed so that it is still the height of its children elements.
